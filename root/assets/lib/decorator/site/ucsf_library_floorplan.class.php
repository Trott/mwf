...
class Ucsf_Library_Floorplan_Site_Decorator extends Content_Site_Decorator {
	 ...
	 */
	public function __construct($title = 'Floor', $params = array(),
    		$mapUrl = '', $legendItems = array(),	$description = '') {
    ...
    public function render() {
            ...
            if (! empty($this->_legendItems)) {
            	$list = '';
           		for ($i = 0, $n = count($this->_legendItems); $i < $n; $i++) {
                    $list .= HTML_Decorator::tag('li', 
                    		    HTML_Decorator::tag('span', 
                    		    	$this->_legendItems[$i]['title'],
        			                $this->_legendItems[$i]['params'])->render()
                    		 )->render();
            		}
            	$section .= HTML_Decorator::tag('ul', $list, array('class' => 'two-col-list'))->render();
            	$section .= HTML_Decorator::tag('br', null, array('class' => 'clear'))->render();
            }
        ...
        if (! empty($this->_description)) {
        	$this->add_section(HTML_Decorator::tag('span', nl2br($this->_description, true), 
